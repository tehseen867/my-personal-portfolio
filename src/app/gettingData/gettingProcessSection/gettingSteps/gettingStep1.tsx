// components/GettingStep1Data.js
...
const GettingStep1Data = () => {
  ...
  useEffect(() => {
    const fetchStep1Data = async () => {
      try {
        const query = `*[_type=="step1"]{
          subHeading,
          heading,
          description,
          keyPoints
          }`;
          const res: Step1DataType[] = await client.fetch(query);
          const latest = res.pop();
        setStep1Data(latest || null);
        setLoading(false)
      } catch (error) {
  ...
  return (
    <div className="xl:px-4 px-2 xl:py-6 py-3 border-[0.2px] border-gray-800 bg-gray-950 rounded-md max-w-[499.6px] w-auto sm:mx-8 xl:mx-14 mx-2 my-3">
      <div className="px-2">
        <p className="text-gray-400 mb-1 sm:text-[15px] text-[10px] font-sans uppercase">{step1Data?.subHeading}</p>
        <h2 className="text-blue-600 font-sans sm:text-2xl text-xl mb-2 uppercase">{step1Data?.heading}</h2>
        <p className="text-white font-extralight xl:text-[15px] text-[12px] font-sans xl:mb-3 mb-2 tracking-wider">{step1Data?.description}</p>
      </div>
      <div>
        {step1Data?.keyPoints.map((keyPoint: string, index: number) => (
...
export default GettingStep1Data